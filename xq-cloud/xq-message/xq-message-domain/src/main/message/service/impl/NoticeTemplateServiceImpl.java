...
import com.share.message.mapper.NoticeTemplateMapper;
import com.share.message.service.IMessageTemplateService;
import com.share.message.service.INoticeTemplateService;
...
public class NoticeTemplateServiceImpl extends ServiceImpl<NoticeTemplateMapper, NoticeTemplate> implements INoticeTemplateService {
    ...
    @Override
    public PageDTO<NoticeTemplateDTO> queryNoticeTemplates(PageSO<NoticeTemplatePageSO> querySO) {
        // 1.分页条件
        Page<NoticeTemplate> page = querySO.toMpPage();

        NoticeTemplatePageSO so = querySO.getCondition();
        // 2.过滤条件
        page = lambdaQuery()
                .eq(so.getStatus() != null, NoticeTemplate::getStatus, so.getStatus())
                .like(StringUtils.isNotBlank(so.getKeyword()), NoticeTemplate::getName, so.getKeyword())
                .page(page);
        // 3.数据转换
